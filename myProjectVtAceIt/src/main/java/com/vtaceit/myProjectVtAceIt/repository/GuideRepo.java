...
public interface GuideRepo extends JpaRepository<Guide, Long> {
    ...
    @Query("Select g from Guide g where g.courseName=?1 and g.courseDept=?2 and g.courseNumber=?3 and g.profName=?4 and g.grade=?5 and g.difficulty=?6 and g.attendanceReq=?7 and g.comments=?8")
    Optional<Guide> alreadyExists(String courseName, String courseDept, Integer courseNumber, String profName, String grade, Integer difficulty, String attendanceReq, String comments);
